...
task main()
{
	...
	MODE mode = getMode();
	int recToPlay = 0;
	if (mode == MODE_PLAY) recToPlay = getRec(p);
	...
	while(time1[T1] < 4000)
	{
		...
		if (leftAlert)
		{
			if (( mode == MODE_REC) && (recOn == false)) recOn = true;
			if (( mode == MODE_PLAY) && (playOn == false)) playOn = true;
			iAlert ++;
			vLeft  -= 5;
			vRight += 5;
