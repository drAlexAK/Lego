...
task main()
{
	...
	while(true)
	{
		...
		if (( abs(e) < 3 ) & ( eCenter1 > 70 ) & ( eCenter2 > 70 ))
		{
#ifdef DEBUG
			playSound(soundBlip);
#endif
			vLeft  = vLeft  - sgn(eSave) * 1;
			vRight = vRight + sgn(eSave) * 1;
			vCenter = vCenter - sgn(eSave) * 1;
		}
		else
		{
			eSave = eSave + u / 100;
			v = vMax - (SensorValue(sCenter1) - esCenter1) / 3.5 - (SensorValue(sCenter2) - esCenter2) / 1 - abs (u) / 5 ;
			vLeft   = v + u ;
			vRight  = v - u ;
			vCenter = u / 1.25;
		}
