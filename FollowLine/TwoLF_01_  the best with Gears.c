...
task main()
{
	...
	float	rwLeft = (rawLight[0] * 1.7 + rawLight[1] * 1.6 + rawLight[2] * 1.5 + rawLight[3] * 1.4 + rawLight[4] * 1.3 + rawLight[5] * 1.2 + rawLight[6]  * 1.1 + rawLight[7]) / 20 * lightCorrect;
	LLreadSensorRaw(sLightRight, rawLight); // read the raw sensor data (8 bit data)
	float	rwRight = (rawLight[0] + rawLight[1] * 1.1 + rawLight[2] * 1.2 + rawLight[3] * 1.3  + rawLight[4] * 1.4 + rawLight[5] * 1.5  + rawLight[6] * 1.6 + rawLight[7] * 1.7) / 20 * lightCorrect;
	es = rwLeft - rwRight;
	int v = 0 ;
	while(true)
	{
		...
		LLreadSensorRaw(sLightLeft, rawLight); // read the raw sensor data (8 bit data)
		rwLeft = rwLeft = (rawLight[0] * 1.7 + rawLight[1] * 1.6 + rawLight[2] * 1.5 + rawLight[3] * 1.4 + rawLight[4] * 1.3 + rawLight[5] * 1.2 + rawLight[6] * 1.1 + rawLight[7]) /20 * lightCorrect;
		LLreadSensorRaw(sLightRight, rawLight); // read the raw sensor data (8 bit data)
		rwRight = (rawLight[0] + rawLight[1] * 1.1 + rawLight[2] * 1.2 + rawLight[3] * 1.3  + rawLight[4] * 1.4 + rawLight[5] * 1.5  + rawLight[6] * 1.6 + rawLight[7] * 1.7) /20 * lightCorrect;
